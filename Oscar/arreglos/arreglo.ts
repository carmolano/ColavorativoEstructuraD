...
function busquedaLineal(arreglo: number[], objetivo: number): number {
...
const valorABuscar: number = numeros[Math.floor(Math.random() * numeros.length)];
const posicion: number = busquedaLineal(numeros, valorABuscar);
